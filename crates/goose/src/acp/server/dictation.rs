...
use crate::dictation::providers::{
    all_providers, get_provider_def, is_configured, transcribe_with_model, transcribe_with_provider,
    DictationProvider,
};
#[cfg(feature = "local-inference")]
...
fn dictation_default_model(provider: DictationProvider) -> Option<String> {
    match provider {
        ...
        DictationProvider::ModelNative => crate::config::Config::global().get_param::<String>("GOOSE_MODEL").ok(),
        #[cfg(feature = "local-inference")]
...
fn dictation_selected_model(config: &Config, provider: DictationProvider) -> Option<String> {
    if provider == DictationProvider::ModelNative {
        return crate::config::Config::global().get_param::<String>("GOOSE_MODEL").ok();
    }
